...
public class Poblacio implements Comparable, Pesable {
	...
	public Poblacio(String nom, int components , int increment) {
		if(nom.equals(null)) {throw new NullPointerException("Valor de nom null");} 
		if(components<=0) {throw new IllegalArgumentException("Valor incorrecte");}
		if(increment<=0) {throw new IllegalArgumentException("Valor incorrecte");}
		this.nom = nom;
	...
	public ContenidorBrossa getContenidor(int quin) {
		if(quin<numContenidors-1) {throw new IndexOutOfBoundsException("Index fora de rang");}
	...
	public String hiEs(String codi) {
		if(codi.equals(null)) {throw new NullPointerException();}
	...
	private static int cercaDicotomica(ContenidorBrossa[] vector, int esquerra, int dreta, ContenidorBrossa c) {
		if(esquerra<0 || dreta>vector.length-1 || dreta<0 ) {throw new IndexOutOfBoundsException("Un dels indexs estan fora de rang");}
		if (dreta >= esquerra) {
	...
	private static int trobar(String carrer, String[]adresa, int quants) {
		if(quants<0) {throw new IllegalArgumentException();}
	...
	public ContenidorBrossa mesRendiment(int tipus) {
		if (tipus!=ContenidorBrossa.GROC && tipus!=ContenidorBrossa.MARRO && tipus!=ContenidorBrossa.GRIS &&
				tipus!=ContenidorBrossa.BLAU &&tipus!=ContenidorBrossa.VERD) {throw new IllegalArgumentException("Valors Incorrectes");}
		String reci ="";
		int major = 0;
		int conversio;
		...
		for (int i =0; i<this.numContenidors; i++) {
			...
			case ContenidorBrossa.MARRO:
				reci = ((Organic) contenidor[i]).getReciclat().replaceFirst(" tones", "");
				conversio = Integer.parseInt(reci);
			...
			case ContenidorBrossa.GRIS:
				reci = ((Rebuig) contenidor[i]).getReciclat().replaceFirst(" tones", "");
				conversio = Integer.parseInt(reci);
			...
			case ContenidorBrossa.VERD:
				reci = ((Vidre) contenidor[i]).getReciclat().replaceFirst(" envasos equivalents", "");
				conversio = Integer.parseInt(reci);
				if (contenidor[i] instanceof Vidre && major<conversio) {
					major = conversio*200/1000;
					c = contenidor[i];
				}
	...
	public Poblacio PoblaciomesCarrer(Poblacio c) {
		if(c == null) {throw new NullPointerException("El valor de població no pot ser null");}
	...
	public boolean equals(Poblacio p) {
		if(p == null) {throw new NullPointerException("El valor de població no pot ser null");}
	...
	public int compareTo(Object c){
		...
		if (! (c instanceof Poblacio)) {
			throw new ClassCastException("El parametre no es una Població");
		}
		else {
			r = (Poblacio) c;
